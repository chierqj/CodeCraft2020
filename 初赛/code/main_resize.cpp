...
#ifdef LOCAL
#define TRAIN "../data/58284/test_data.txt"
#define RESULT "../data/58284/result.txt"
#else
...
class XJBG {
 private:
  ...
  // std::unordered_set<int> m_StepArrive[MAXN];
  std::vector<std::unordered_set<int>> m_StepArrive;
 ...
 private:
  ...
  std::vector<std::vector<Edge>> m_Edges;  // 边
  int m_edgeNum = 0;                       // 边数目
  // int m_dfn[MAXN], m_low[MAXN], m_stack[MAXN];  // Tarjan
  std::vector<int> m_dfn, m_low, m_stack;  // Tarjan
  // int m_catrgory[MAXN];                     // 所在联通分量id
  std::vector<int> m_catrgory;  // 所在联通分量id
  // int m_inDegree[MAXN], m_outDegree[MAXN];  // 出入度
  std::vector<int> m_inDegree, m_outDegree;  // 出入度
  // bool m_inStack[MAXN];                      // Tarjan标记在不在栈内
  std::vector<int> m_inStack;  // Tarjan标记在不在栈内
  // bool m_vis[MAXN];             // 找环标记
  std::vector<int> m_vis;  // 找环标记
  // bool m_inCircle[MAXN];        // 在不在找过的环内
  std::vector<int> m_inCircle;  // 在不在找过的环内
...
void XJBG::ResizeData() {
  ...
  std::vector<int> m_inDegree, m_outDegree;  // 出入度
  // bool m_inStack[MAXN];                      // Tarjan标记在不在栈内
  std::vector<bool> m_inStack;  // Tarjan标记在不在栈内
  // bool m_vis[MAXN];             // 找环标记
  std::vector<bool> m_vis;  // 找环标记
  // bool m_inCircle[MAXN];        // 在不在找过的环内
  std::vector<bool> m_inCircle;  // 在不在找过的环内
  */
  int maxn = m_IDDom.size();

  m_Edges.resize(maxn);
  m_StepArrive.resize(maxn);
  m_dfn.resize(maxn);
  m_low.resize(maxn);
  m_stack.resize(maxn);
  m_catrgory.resize(maxn);
  m_inDegree.resize(maxn);
  m_outDegree.resize(maxn);
  m_inStack.resize(maxn);
  m_vis.resize(maxn);
  m_inCircle.resize(maxn);

  // m_Edges = std::vector<std::vector<Edge>>(maxn);
  // m_StepArrive = std::vector<std::unordered_set<int>>(maxn);
  // m_dfn = std::vector<int>(maxn);
  // m_low = std::vector<int>(maxn);
  // m_stack = std::vector<int>(maxn);
  // m_catrgory = std::vector<int>(maxn);
  // m_inDegree = std::vector<int>(maxn);
  // m_outDegree = std::vector<int>(maxn);
  // m_inStack = std::vector<int>(maxn);
  // m_vis = std::vector<int>(maxn);
  // m_inCircle = std::vector<int>(maxn);
...
void XJBG::LoadData() {
  ...
  while (start < bufsize) {
    if (*(buffer + start) == ',') {
      ...
      U.emplace_back(this->GetMapID(temp[0]));
      V.emplace_back(this->GetMapID(temp[1]));
      W.emplace_back(x);
