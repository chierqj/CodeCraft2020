...
struct RadixHeapUShort {
  ...
  inline static constexpr int pos(ushort x) {
    return x == 0 ? 0 : 16 - __builtin_clz(x);
  }
...
struct SolverDataUShort {
  ...
  inline void modify(uint x, ushort nv) {
...
void ZKW(SolverDataUint &Data, const uint &start) {
  uint(&m_points)[MAX_NODE] = Data.points;
  uint(&m_count)[MAX_NODE] = Data.count;
  uint(&m_dis)[MAX_NODE] = Data.dis;
  double(&m_ans)[MAX_NODE] = Data.ans;
  ...
  m_count[start + 1] = 1;      // 初始化dijkstra
  Data.M = Data.P;             // 初始化ZKW
  Data.id[0] = g_NodeNum + 1;  // 初始化ZKW
  Data.modify(start + 1, 0);   // 初始化ZKW
  ...
  for (uint t = 1; t <= g_NodeNum; ++t) {
    ...
    *++pointptr = u;  //拓扑序
    ...
    const uint &ucount = m_count[u];
    const uint &udis = m_dis[u];
    const auto &l = Head[u - 1], &r = Head[u];
    const DFSEdge *e = &GHead[l];
    for (uint i = l; i < r; ++i, ++e) {
      const uint &v = e->idx + 1;
      const uint &w = e->w;
      const uint &newdis = udis + w;
      auto &vdis = m_dis[v];
...
void ZKW(SolverDataULong &Data, const uint &start) {
  uint(&m_points)[MAX_NODE] = Data.points;
  uint(&m_count)[MAX_NODE] = Data.count;
  ulong(&m_dis)[MAX_NODE] = Data.dis;
  double(&m_ans)[MAX_NODE] = Data.ans;
...
void UnionSet() {
  ...
  for (uint i = 0; i < g_EdgeNum; ++i) {
    ...
    uint x = getFa(e.u);
    cnt[x] += e.w;
    if (cnt[x] > uint_max) IfNeedULong = true;
