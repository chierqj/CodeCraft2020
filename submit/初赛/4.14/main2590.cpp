...
class XJBG {
 ...
 private:
  ...
  inline void connectBuffer(int dep, int node, char c);
...
inline void XJBG::connectBuffer(int dep, int node, char c) {
  ...
  memcpy(m_AnsBuf[dep] + m_AnsBufLen[dep], mpid.str + mpid.start, mpid.length);
  m_AnsBufLen[dep] += mpid.length;
  m_AnsBuf[dep][m_AnsBufLen[dep]++] = c;
}
...
void XJBG::FindCircle(int pid) {
  ...
  for (int i = 0; i < 5; ++i) {
    m_AnsBuf[i] = new char[3000000 * (i + 3) * 11];
    m_AnsBufLen[i] = 0;
  }
  ...
  for (int i = 0; i < 5; ++i) {
    *m_TotalBufferSizePtr += m_AnsBufLen[i];
  }
