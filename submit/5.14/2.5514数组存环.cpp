...
const uint BUFFERBLOCK = 128;      // 输出分块
...
struct ThData2 {
  ...
  uint ReachPoint[MAXN];                   // 可达点集合
  uint LastWeight[MAXN];                   // 最后一步权重
  uint8_t Path2Cnt[MAXN], Path3Cnt[MAXN];  //
  std::array<uint, 2> Path2[MAXN][128];
  std::array<uint, 3> Path3[MAXN][128];
};
...
void BackSearch2(ThData2 *&Data, const uint &st) {
  ...
  for (uint it1 = Back[st]; it1 < Back[st + 1]; ++it1, ++e1) {
    ...
    for (uint it2 = Back[v1]; it2 < Back[v1 + 1]; ++it2, ++e2) {
      ...
      Data->Reach[v2] |= 6;
      Data->ReachPoint[Data->ReachCount++] = v2;
      Data->Path2[v2][Data->Path2Cnt[v2]++] = std::array<uint, 2>{v1, w2};
      ...
      for (uint it3 = Back[v2]; it3 < Back[v2 + 1]; ++it3, ++e3) {
        ...
        Data->Reach[v3] |= 4;
        Data->ReachPoint[Data->ReachCount++] = v3;
        Data->Path3[v3][Data->Path3Cnt[v3]++] = std::array<uint, 3>{v2, v1, w3};
      }
    }
  }
}
      ...
      for (uint it3 = Head[v2]; it3 < Head[v2 + 1]; ++it3, ++e3) {
        ...
        for (uint it4 = Head[v3]; it4 < Head[v3 + 1]; ++it4, ++e4) {
          ...
          if (!Data->vis[v4]) {
            Data->vis[v4] = true;
            std::sort(
                Data->Path2[v4], Data->Path2[v4] + Data->Path2Cnt[v4],
                [&](const std::array<uint, 2> &p1,
                    const std::array<uint, 2> &p2) { return p1[0] < p2[0]; });
            std::sort(Data->Path3[v4], Data->Path3[v4] + Data->Path3Cnt[v4],
          ...
          const uint8_t &cnt2 = Data->Path2Cnt[v4];
          ...
          const uint8_t &cnt3 = Data->Path3Cnt[v4];
          for (uint i = 0; i < cnt3; ++i) {
